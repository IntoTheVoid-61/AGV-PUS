 ...
 *
 * This program implements a ROS 2 node that declares parameters for a AGV robot 
 ...
 * 
 * Specied velocity commands are with respect to the robots coordinate system a.k.a base_link/base_footprint
 * Therefore: x is forward/backwards, y is left/right and z is CCW/CW   (positive/negative)
 * 
 * 
 * 
 * You can use this to command the velocity of AGV, more specifically you can command:
 *  linear velocity in x: target_linear_velocity_x
 *  linear velocity in y: target_linear_velocity_y
 *  angular velocity in z: target_angular_velocity
 *
 * Subscription Topics:
 *   None
 *
...
public:
  ...
  MecanumParameters() : Node("mecanum_parameters_node")
  {
    ...
    this->declare_parameter("robot_name", "AGV-PUS", robot_name_descriptor);
